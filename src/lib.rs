...
//! private room keys and share links.
...
impl Peersey {
    ...
    /// Create and join a new private pub/sub room.
    pub async fn create_room(&self) -> Result<(Room, RoomKey), Error> {
        let key = RoomKey::random();
        let room = Room::join(key).await?;
        Ok((room, key))
    }

    /// Join a private pub/sub room using its secret capability key.
    pub async fn join_room(&self, key: RoomKey) -> Result<Room, Error> {
        Room::join(key).await
    }
